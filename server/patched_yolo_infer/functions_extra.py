...
class Segment_Stream_Class:
    ...
    def visualize_results_usual_yolo_inference(self,
        img,
        # model,
        imgsz=640,
        conf=0.5,
        ...
        for pred in predictions:
            ...
            for i in range(num_objects):
                ...
                if random_object_colors:
                    color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
                elif self.list_of_class_colors is None:
                    # Assign color according to class
                    random.seed(int(classes[i] + delta_colors))
                    color = (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
                else:
                    color = self.list_of_class_colors[classes[i]]
