...
class LoadStreams:
    ...
    def __init__(self,  imgsz=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        # n = len(sources)
        # self.self.sources = [ops.clean_str(x) for x in sources]  # clean source names for later
        self.imgs, self.fps, self.frames, self.threads, self.cap = [None] * 1, [0] * 1, [0] * 1, [None] * 1, [None] * 1
    ...
    def newThreads(self, sources):
        ...
        # Check for common shapes
        s = np.stack([LetterBox(self.imgsz, self.auto, stride=self.stride)(image=x).shape for x in self.imgs])
        self.rect = np.unique(s, axis=0).shape[0] == 1  # rect inference if all shapes equal
        self.auto = self.auto and self.rect
        self.transforms = self.transforms  # optional
        self.bs = self.__len__()
