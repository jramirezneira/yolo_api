...
class FaceAlignment:   
    ...
    def frontalize_face(self, face_rect, landmarks,  _frame):
        # get the landmarks
        ...
        #     shape5=convert_to_numpy(shape5)
        landmarks=convert_to_numpy(landmarks)
        # points_dlib68 = [dlib.point(int(x), int(y)) for (x, y) in landmarks]
        # for i, p in enumerate(points_dlib68):
        #     cv2.circle(_frame, (p.x, p.y), 3, (0, 255, 0), -1)
